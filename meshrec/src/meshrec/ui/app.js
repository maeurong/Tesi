...
// un'altra cosa e sta apposta su un'altra strada, con un altro nome.
//
...
// di primo livello come superata() e valoreScritto(): e' l'unico punto in cui
// una combinazione di tasti diventa una scrittura su disco, e da fuori si
// prova senza un motore di DOM.
// metaKey oltre a ctrlKey: su macOS il gesto e' cmd+z, e questo progetto e' su
...
function fraseDelRitorno(prima, dopo) {
  ...
  const passatiA = (stato) =>
    dopo
      .filter((voce) => voce.stato === stato && era.get(voce.numero) !== stato)
      .map((voce) => nomeDelloStep(voce.numero, dopo));
...
// bumparla scarta ogni tratta in volo: mostraStep sui rami superati torna senza
// chiudere il caricamento, perche' conta su chi ha bumpato per ripartire con un
// caricamento nuovo. Questa e' la prima strada che puo' uscire senza ripartire
// — «niente da annullare», guasto, corpo nullo, rifiuto — e la tela resterebbe
// con aria-busy e la didascalia «caricamento di...» finche' non si clicca
// altrove. E' lo stesso motivo per cui la geometria ha ultimaGeometria.
let ultimoRitorno = 0;
...
async function chiediStorico(verso) {
  ...
  // finiscono in volo insieme, e aperto dopo l'ordine sarebbe il numero di
  // arrivo invece che quello di partenza, cioe' vincerebbe la risposta vecchia.
  ...
  // dettaglio, che apriDettaglio svuota a ogni apertura (dichiaraErrore(null)),
  // cioe' proprio qualche riga piu' sotto: scritti la', due messaggi su cinque
  // sparirebbero nel tempo di due fetch. dichiaraErrore non fa altro che
  // scrivere quel testo — nessun comando da spegnere, nessuno stato da azzerare
  // — quindi qui non serve chiamarla comunque.
  if (!risposta.ok) {
    const ragione = await ragioneDelRifiuto(risposta);
  ...
  // di un ritorno riuscito, che e' il difetto gia' prodotto e corretto una
  // volta sul bottone Annulla.
  if (!corpo.annullato) {
    ...
    if (corpo.guasto) mostraEsito(corpo.perche, null);
    else mostraEsito(null, corpo.perche);
    return;
  }
  // Composta prima di caricaStato(), che riscrive ultimiSteps: il termine di
  ...
  // dirlo e' il punto, perche' un ritorno che cambia in silenzio lo stato di
  // sette step e' una modifica invisibile.
  mostraEsito(null, fraseDelRitorno(ultimiSteps, corpo.steps));
  await caricaStato();
  ...
  // batte.
  if (stepMostrato !== null) ricaricaVista(stepMostrato);
  if (stepAperto !== null) apriDettaglio(stepAperto);
}
...
// gestore globale su quelli li rubere'bbe a chi orbita da tastiera.
