...
| gmsh | 4.15.2 | PASS | ottimizzatore post-mesh **opzionale** accanto a TetGen, da validare in Fase 1 su geometrie reali |
...
- Ottimizzazione della qualità: **Gmsh** (`optimize("Netgen")`), opzionale — non richiesto per far girare la pipeline principale. Sulla geometria sintetica di prova la qualità minima è passata da 0,037787 a 0,423365, ma il numero di elementi è cambiato da 540 a 775 (+43,5%): il confronto non è a parità di elementi, e su point cloud reali (rumorose, non watertight in origine) la riclassificazione delle superfici da STL è il passaggio più a rischio — da validare in Fase 1 su dati reali, non solo sintetici.
