...
- **Nessun numero del provino di laboratorio in `src/`.** La regola nasce da F10 della Fase 4 (`docs/fase-4-cantiere/correzioni-finali-report.md`), che tolse «176 su lab_frame, 1245.7 su muro_generato» dal docstring di `known_thickness`. Colpisce le **grandezze del provino** — dimensioni, sezioni, materiale: i valori che l'operatore deve dichiarare e che un docstring non deve suggerire. **Non** colpisce le misure di *comportamento della pipeline* su quei dati, che sono la calibrazione di una scelta e stanno in `src/` per prassi consolidata (`abaqus.py:488`, `config.py:227`, `quality.py:336`). I numeri dei banchi sintetici stanno nei test.
