...
| O3 | `export["volume"]` | `abaqus.export_model` | **chiuso** (`aa2716f`): scatola nota, rel 1e-6 — `test_oracoli_mancanti.test_il_volume_del_deck_e_quello_della_scatola`. `export["mass"]` non esiste piu': il deck nudo non porta materiale |
| O4 | GRAVITY_MM_S2 = 9810.0 | uscita dalla configurazione | **superato il 08/09/2026**: il deck nudo non scrive la gravità, e la costante è uscita con AnalysisConfig (vedi ADR). Chiuso era (`aa2716f`), asserita contro 9,81·1000 e ρ·V·g → N sull'acqua; le due prove escono con la costante |
