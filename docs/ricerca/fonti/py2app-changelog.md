...
- #564: Fix broken support for setuptools 82: - pkg\_resources was removed, preparation in 0.28.9 was incomplete - a number of utility functions in setuptools lost there ‘dry\_run’ argument..
- Fix test failures with setuptools 80.
- This release requires python 3.10 or later
	The code should still work with older versions, but I no longer test with those versions.
...
- It is no longer possible to use an `install_requires` option when invoking py2app.
	Support for this was inherited from setuptools, and that feature has been deprecated for a long time at this point.
- Don’t use `pkg_resources`
	The codebase still falls back to using `pkg_resources` for ancient versions of Python, but should no longer use this library for versions that are still in support.
...
- #476: Update black recipe
	...
	The recipe now scans the python code that’s next to the mypyc compiled extension modules for dependencies and uses that to update the dependency graph. This should ensure that new dependencies of black will be automaticly detected in the future.
...
> [!note] Note
> This is the last version of py2app with compatibility with Python 2.7. Future versions will require Python 3.6 or later.

- PR #410: Fix typo in NamedTemporyFile call
	PR by MAKOMO
- #414 Workaround for autodiscovery in setuptools 61.0
	Setuptools 61.0 introduces autodiscovery of distribution attributes, and that broke py2app. This version introduces a `setuptools.finalize_distribution_options` entrypoint in py2app that will set the distributions’s *name* and *py\_modules* attributes in a way that is compatible with the main code of py2app when they are not yet set (before autodiscovery kicks in).
...
- #413: Find dist-info in included pythonXX.zip
	By default the `working_set` of pkg\_resources does not contain distribution information from packages included in zip files, such as the zipped-up stdlib + site-pakckages in py2app bundles.
...
- PR #388: Add builtin definitions for ‘quit’ and ‘exit’ in site.py
	PR by mcclary
- PR #388: Set “ENABLE\_USER\_SITE=False” in site.py
	PR by mcclary
...
- #368: Add recipe “detect\_dunder\_file”
...
- #338: Add “imageio\_ffmpeg” to autopackages
...
- #371: change default error message on launch problems
	The default error message shown when the application cannot be launched is now slightly more useful and refers the [py2app debug page](https://py2app.readthedocs.io/en/latest/debugging.html).
...
- #315: Stub executables have an LC\_RPATH that points to the Frameworks folder
...
- #320: Process “@loader\_path” in load commands
	A popular pattern in C extensions with bindins to C library on PyPI is to copy those C libraries into the wheel and reference those using an “@loader\_path” linker command in the C extension. Until this release py2app could not process those linker commands correctly.
...
- Make sure the stdout/stderr streams of the main binary of the application are unbuffered.
	See [issue #177 in PyObjC’s repository](https://github.com/ronaldoussoren/pyobjc/issues/177/on-python3-print-does-not-automatically) for more information.
...
- py2applet now longer uses “argv\_emulation” by default, that results in too many problems.
...
- Add support for “@loader\_path” in the link commands of C extension.
	This makes it possible to use wheels that were processed by [delocate-listdeps](https://github.com/matthew-brett/delocate) when building application bundles.
...
- Report modules that contain syntax errors at the end of the run (for non-alias builds)
	Note: This is just a warning, syntax errors be valid when the dependency tree contains modules for the other major release of python (e.g a compat\_py2 module that contains compatibility code for Python 2 and contains code that isn’t valid Python 3)
...
- Issue #80: Add support for copying system plugins into the application bundle.
	Py2app now supports a new option *include\_plugins*. The value of this is a list of paths to plugins that should be copied into the application bundle.
...
- The i386, x86\_64 and intel stub binaries are now compiled with clang on OSX 10.8, instead of an older version of GCC. The other stub versions still are compiled on OSX 10.6.
- Issue #111: The site.py generated by py2app now contains a USER\_SITE variable (with a default value of `None`) because some software tries to import the value.
...
- Issue #101: Set “PYTHONDONTWRITEBYTECODE” in the environment before calling Py\_Initialize to ensure that the interpreter won’t try to write bytecode files (which can cause problems when using sandboxed applications).
...
- Issue #88: Ensure that the fix for #65 won’t try to create a symlink that points to itself. This could for example occur with homebrew, where the exposed lib directory contains symlinks to a cellar, while tye install\_name does mention the “public” lib directory:
	```
...
- Always include ‘pkg\_resources’, this is needed to correctly work with setuptools namespace packages, the \_\_init\_\_.py files of those contain `__import__('pkg_resources')` and that call isn’t recognized as an import by the bytecode scanner.
- Issue #67: py2applet didn’t work with python 3 due to the use of ‘raw\_input’
...
- Smarter matplotlib recipe, it is now possible to specify which backends should be included. Issue #44, reported by Adam Kovics.
	The argument to `--matplotlib-backends` (or ‘matplotlib\_backends’ in setup.py) is a list of plugins to include. Use ‘-’ to not include backends other than those found by the import statement analysis, and ‘\*’ to include all backends (without necessarily including all of matplotlib)
...
- issue #18: improved PyQt and PySide support.
	Py2app now has a new option named “–qt-plugins” (or “qt\_plugins” in setup.py), this option specify a list of plugins that should be included in the application bundle. The items of the list can have a number of forms:
	...
	The plugins are copied into “Resources/qt\_plugins” and py2app adds a “qt.conf” file that points to that location for plugins.
...
- fix mixed indentation (tabs and spaces) in argv\_emulation.py, which caused installation failures on python 3.x (issue #40)
...
- on some OSX releases the application receives both the “open application” and “open documents” Apple Events during startup, which broke an assumption in argv\_emulation.py.
...
- Issue #28: the argv emulator crashes in 64-bit mode on OSX 10.5
	Fixing this issue required yet another rewrite of the argv\_emulator code.
- Added option ‘–arch=VALUE’ which can be used to select the set of architectures for the main executable. This defaults to the set of architectures supported by the python interpreter and can be used to drop support for some architectures (for example when you’re using a python binary that supports both 32-bit and 64-bit code and use a GUI library that does not yet work in 64-bit mode).
	...
	- fat: i386, ppc
		- fat3: i386, x86\_64, ppc
		- univeral: i386, x86\_64, ppc, ppc64
		- intel: i386, x86\_64
...
- Issue #17: py2applet script defaults ‘argv\_emulation’ to False when your using a 64-bit build of python, because that option is not supported on such builds.
...
- it is now possible to specify which python distributions must be availble when building the bundle by using the “install\_requires” argument of the `setup()` function:
	```
	setup(
	    ...
	    install_requires = [
...
- Avoid copying the \_\_pycache\_\_ directory in python versions that implement PEP 3147 (Python 3.2 and later)
...
- py2app now actually works for me ([ronaldoussoren@mac.com](mailto:ronaldoussoren%40mac.com)) with a python interpreter in a virtualenv environment.
...
- A bad format string in build\_app.py made it impossible to copy the Python framework into an app bundle.
...
- A new recipe for [PyQt](http://www.riverbankcomputing.co.uk/pyqt/index.php) 4.x. This recipe was donated by Kevin Walzer.
- A new recipe for [virtualenv](http://pypi.python.org/pypi/virtualenv), this allows you to use py2app from a virtual environment.
...
- Resolve disable\_linecache issue
...
- Application bootstrap now detects paths containing “:” and will provide a “friendly” error message instead of just crashing < [http://python.org/sf/1507224](http://python.org/sf/1507224) >.
...
- macholib, bdist\_mpkg, modulegraph, and altgraph are now separately maintained packages available on PyPI as eggs
...
- macholib.util.in\_system\_path understands SDKs now
- DYLD\_LIBRARY\_PATH searching is fixed
...
- bdist\_mpkg now builds packages with the full platform info, so that installing a package for one platform combination will not look like an upgrade to another platform combination.
...
- bdist\_mpkg now builds zip files that are correctly unzipped by all known tools.
...
- bdist\_mpkg now compatible with pychecker (data\_files had issues)
...
- Looks for a hacked version of the PyOpenGL \_\_init\_\_.py so that it doesn’t have to include the whole package in order to get at the stupid version file.
...
- bdist\_mpkg now has a `--zipdist` option to build zip files from the built package
- The bdist\_mpkg “Installed to:” description is now based on the package install root, rather than the build root

## py2app 0.1.7

[py2app](http://undefined.org/python/#py2app) 0.1.7 is a bug fix release:
...
- `py2app` ’s not\_stdlib\_filter will now ignore a `site-python` directory as well as `site-packages`.
...
[py2app](http://undefined.org/python/#py2app) 0.1.6 was a major feature enhancements release:
...
- `py2app` can now create plugin bundles (MH\_BUNDLE) as well as executables. New recipe for supporting extensions built with [sip](http://www.riverbankcomputing.co.uk/sip/index.php), such as [PyQt](http://www.riverbankcomputing.co.uk/pyqt/index.php). Note that due to the way that [sip](http://www.riverbankcomputing.co.uk/sip/index.php) works, when one sip-based extension is used, *all* sip-based extensions are included in your application. In practice, this means anything provided by [Riverbank](http://www.riverbankcomputing.co.uk/), I don’t think anyone else uses [sip](http://www.riverbankcomputing.co.uk/sip/index.php) (publicly).
- New recipe for [PyOpenGL](http://pyopengl.sourceforge.net/). This is very naive and simply includes the whole thing, rather than trying to monkeypatch their brain-dead version acquisition routine in `__init__`.
...
- Refactored `macholib` to use `altgraph`, can now generate [GraphViz](http://www.pixelglow.com/graphviz/) graphs and more complex analysis of dependencies can be done.
- `macholib` was refactored to be easier to maintain, and the structure handling has been optimized a bit.
- The few tests that there are were refactored in [py.test](http://codespeak.net/py/current/doc/test.html) style.
- New [PyQt](http://www.riverbankcomputing.co.uk/pyqt/index.php) example.
- New [PyOpenGL](http://pyopengl.sourceforge.net/) example.

See also:

- [http://mail.python.org/pipermail/pythonmac-sig/2004-December/012272.html](http://mail.python.org/pipermail/pythonmac-sig/2004-December/012272.html)

## py2app 0.1.5

[py2app](http://undefined.org/python/#py2app) 0.1.5 is a major feature enhancements release:
...
- Added a `bdist_mpkg` distutils extension, for creating Installer an metapackage from any distutils script.
	- Includes PackageInstaller tool
		- bdist\_mpkg script
		- setup.py enhancements to support bdist\_mpkg functionality
- Added a `PackageInstaller` tool, a droplet that performs the same function
	as the `bdist_mpkg` script.
- Create a custom `bdist_mpkg` subclass for [py2app](http://undefined.org/python/#py2app) ’s setup script.
- Source package now includes [PJE](http://dirtsimple.org/) ’s [setuptools](http://cvs.eby-sarna.com/PEAK/setuptools/) extension to distutils.
...
- New [docutils](http://docutils.sf.net/) recipe. Support for non-framework Python, such as the one provided by [DarwinPorts](http://darwinports.opendarwin.org/).

See also:

- [http://mail.python.org/pipermail/pythonmac-sig/2004-October/011933.html](http://mail.python.org/pipermail/pythonmac-sig/2004-October/011933.html)

## py2app 0.1.4

[py2app](http://undefined.org/python/#py2app) 0.1.4 is a minor bugfix release:
...
[py2app](http://undefined.org/python/#py2app) 0.1.3 is a refactoring and new features release:

- `altgraph`, my fork of Istvan Albert’s [graphlib](http://www.personal.psu.edu/staff/i/u/iua1/python/graphlib/html/), is now part of the distribution
- `py2app.modulegraph` has been refactored to use `altgraph`
- [py2app](http://undefined.org/python/#py2app) can now create [GraphViz](http://www.pixelglow.com/graphviz/) DOT graphs with the `-g` option ([TinyTinyEdit example](http://undefined.org/~bob/TinyTinyEdit.pdf))
...
[py2app](http://undefined.org/python/#py2app) 0.2 is primarily a bugfix release:
...
- A [wxPython](http://www.wxpython.org/) example demonstrating how to package [wxGlade](http://wxglade.sourceforge.net/) has been added (this is a good example of how to write your own recipe, and how to deal with complex applications that mix code and data files)
...
[py2app](http://undefined.org/python/#py2app) 0.1.1 is primarily a bugfix release:
...
- A recipe has been added for [py2app](http://undefined.org/python/#py2app) itself
- a [wxPython](http://www.wxpython.org/) example (superdoodle) has been added. Demonstrates not only how easy it is (finally!) to bundle [wxPython](http://www.wxpython.org/) applications, but also how one setup.py can deal with both [py2exe](http://starship.python.net/crew/theller/py2exe/) and [py2app](http://undefined.org/python/#py2app).
...
(first public release) [py2app](http://undefined.org/python/#py2app) is the bundlebuilder replacement we’ve all been waiting for. It is implemented as a distutils command, similar to [py2exe](http://starship.python.net/crew/theller/py2exe/).