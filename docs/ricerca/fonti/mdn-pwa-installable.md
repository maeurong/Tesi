...
**Note:** While not a requirement for a PWA to be installable, many PWAs use [service workers](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API) to provide an offline experience. See the [CycleTracker: Service workers](https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps/Tutorials/CycleTracker/Service_workers) tutorial for more information.
...
- [How to publish a PWA to the Google Play Store](https://chromeos.dev/en/publish/pwa-in-play "External link (opens in new tab)")
- [How to publish a PWA to the Microsoft Store](https://learn.microsoft.com/en-us/microsoft-edge/progressive-web-apps/how-to/microsoft-store "External link (opens in new tab)")
- [How to publish a PWA to the Meta Quest Store](https://developers.meta.com/horizon/resources/publish-submit/ "External link (opens in new tab)")

The [PWABuilder](https://docs.pwabuilder.com/#/builder/quick-start "External link (opens in new tab)") is a tool to simplify the process of packaging and publishing a PWA for various app stores. It supports the Google Play Store, Microsoft Store, Meta Quest Store, and iOS App Store.
...
- Safari supports Add to Dock (*File* > *Add to Dock...*) on macOS Sonoma (Safari 17) and later for any web app with or without a manifest file.
- Firefox does not support installing PWAs using a manifest file.

On mobile:

- On Android, the only browsers that [install PWAs as WebAPKs](https://web.dev/learn/pwa/installation#webapks "External link (opens in new tab)") are: Chrome on devices with Google Mobile Services (GMS), and Samsung Internet on Samsung devices. This gives them a real entry in the app launcher and switcher, and in the system settings. Firefox, Edge, Opera, and other browsers including Chrome on devices where GMS is not present, instead add a browser-badged home-screen shortcut that opens the site in the browser.
...
The screenshot below shows what the install prompt for the [PWAmp demo](https://github.com/MicrosoftEdge/Demos/tree/main/pwamp "External link (opens in new tab)") looks like on Google Chrome, running on Android:
...
You can use the [`display`](https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps/Manifest/Reference/display) manifest member to control the *display mode*: that is, how the PWA appears when it is launched. In particular:
